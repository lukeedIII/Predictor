...
def selective_scan(x: torch.Tensor, delta: torch.Tensor, A: torch.Tensor,
    ...
    """
    Chunked selective scan (S6) — massively reduced kernel dispatch overhead.

    Instead of launching 2 CUDA kernels per timestep (L=120 → 240 launches),
    processes CHUNK_SIZE steps per Python iteration.  Each chunk does ~6 ops
    → for L=120, chunks of 8 → 15 iterations × 6 ops = 90 launches (vs 240).
    The recurrence h[t] = A_bar*h[t-1] + B_bar*x[t] is still sequential
    between chunks, but within-chunk math is fused by the JIT.
...
class MoELayer(nn.Module):
    ...
    def forward(self, x):
        """Vectorized MoE — no Python loops over experts.

        Uses stacked weight matrices + torch.bmm so all experts run in
        a single batched GEMM, then scatters results back by routing mask.
        """
        B, L, D = x.shape
        x_flat = x.view(-1, D)  # (B*L, D)
        T = x_flat.shape[0]     # total tokens
        ...
        tokens_per_expert = one_hot.sum(0) / T
        ...
        # For top_k=1 (our case), each token goes to exactly one expert.
        # Group tokens by expert, run each expert's SwiGLU, scatter back.
        output = torch.zeros_like(x_flat)
        expert_ids = top_k_indices[:, 0]  # (T,) — which expert per token

        for i in range(self.n_experts):
            mask = (expert_ids == i)
            if not mask.any():
                continue
            inp = x_flat[mask]                           # (n_tok, D)
            gate = F.silu(self.gate_projs[i](inp))
            up = self.up_projs[i](inp)
            out = self.down_projs[i](gate * up)
            output[mask] = top_k_probs[mask, 0:1] * out
